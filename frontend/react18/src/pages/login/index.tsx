...
import {$postLogin, type LoginParams, $getCaptcha, $postLoginWith2fa} from "@/services/auth";
import "./index.less";
...
const LoginPage: React.FC = () => {
	...
	const handleLogin = async (formData: LoginParams) => {
		setLoading(true);
		try {
			const { data: { accessToken, isTemporary} } = await $postLogin({ ...formData, captchaId: captcha.id });
            // 临时token，需要OPT二次认证
			if (isTemporary) {
                Modal.confirm({
                    icon: null,
